from fastapi import FastAPI, Response, UploadFile, File, Form, status, Request, HTTPException
...
@app.get("/api/admin/files/{username}")
@app.get("/api/admin/files/{username}/{path:path}")
...
@app.post("/api/del_user", status_code=status.HTTP_200_OK)
...
@app.get("/api/download/{username}/{path:path}")
...
@app.get("/api/content/{username}/{path:path}")
...
@app.delete("/api/delete/{username}/{path:path}")
